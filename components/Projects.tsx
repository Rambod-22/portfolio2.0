        ...
        <motion.div initial={{opacity: 0}} whileInView={{opacity: 1}} transition={{duration:1.5}}  className='h-screen relative flex overflow-hidden flex-col text-left md:flex-row max-w-full justify-evenly mx-auto items-center z-0'>
            ...
            <div ref={scrollRef} className='scrollbar scrollbar-track-gray-400/20 scrollbar-thumb-[#F7AB0A]/80 relative w-full flex overflow-x-scroll overflow-y-hidden snap-x snap-mandatory z-20 pb-10'>
                {projects.map((project, i) => (
                    <div key={project._id} className='w-screen flex-shrink-0 snap-center flex flex-col space-y-8 items-center justify-center p-20 md:p-44 h-screen'>
                        <motion.img className='h-80 md:h-96' initial={{y:-300, opacity: 0}} transition={{duration: 1.2}} whileInView={{opacity: 1, y:0}} viewport={{once:true}} src={urlFor(project?.image).url()} alt="" />
                        <div className='space-y-6 px-4 md:px-10 max-w-6xl'>
                            <h4 className='text-3xl md:text-4xl font-semibold text-center leading-tight'><span className='underline decoration-[#F7AB0A]/50'>Case Study {i + 1} of {projects.length}:</span>{" "}{project.title}</h4>
